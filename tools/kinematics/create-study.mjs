...
export async function createStudy(directory,machineId='split-delta',{source,model={}}={}){
  ...
  const plan={schema:'saam-machine-study/1',output:split?'split-delta-preview':'machine-study',setup,placement:{xMm:0,yMm:0},studyBounds:bounds,
  ...
  for(const [name,data] of [['plan.json',plan],['machine.json',machine],[split?'motion.sdgcode':'motion.json',source]])await writeFile(resolve(dir,name),typeof data==='string'?data:JSON.stringify(data,null,2)+'\n');
...
if(process.argv[1]&&resolve(process.argv[1])===fileURLToPath(import.meta.url)){
  ...
  const source=text===undefined?undefined:sourceFile.endsWith('.sdgcode')?text:JSON.parse(text),model=modelFile?JSON.parse(await readFile(modelFile,'utf8')):{};
