...
export async function loadBundle(dir,{program=true,allSources=false}={}){
  ...
  if(checkedStudy?.source!==source)checkedStudy={source,program:withTravelAdvisory(interpretMachineStudy(source))};
  const decoded=checkedStudy.program,revision=hash(planText+machineText),exportHash=hash(source),bounds=plan.studyBounds;
