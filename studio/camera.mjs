...
export function createProjection(bounds,width,height,yaw,tilt,zoom,pan=[0,0]) {
  ...
  const project=point=>{
    ...
    return [width/2+pan[0]+u*scale,height*.53+pan[1]-(v*sinTilt+z*cosTilt)*scale,-v*cosTilt+z*sinTilt];
  };
  // CSS pixels per model millimeter; the canvas transform separately handles
  // device pixels. Use the same scale for deposited line width and positions.
  project.pixelsPerMm=scale;
  return project;
}
