...
|---|---|---|
| Design file | The saved geometry, and editable design information where available. | Native mesh is the requested new direction (D-021); existing bundles still use Rhino/3DM. |
...
| Planar infill | Flat layers with walls and a sparse interior pattern. | Suggested skill name `planar-infill`; design only. Solid regions would reuse full-fill. |
