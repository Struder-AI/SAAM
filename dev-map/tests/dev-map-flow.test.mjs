...
import {loadFlow,flowPacket,flowPage} from '../lib/flow.mjs';
...
test('a factory-returned object links its members, through a name, a destructuring or two definitions',async()=>{
  ...
  // A stable direct-record selection is now resolved during ordinary call extraction.
  assert.deepEqual(links.filter(l=>l[0]==='core/use.mjs::destructured'),
    [['core/use.mjs::destructured','core/session.mjs::sourceSession','ast-call-site',false],
      ['core/use.mjs::destructured','core/session.mjs::save','ast-call-site',false]]);
...
import {generate,readGenerated,readCode,storeDir} from '../lib/store.mjs';
...
// ---- external facts ------------------------------------------------------------------------
import {readFacts,factsPath} from '../lib/facts.mjs';
import {storeStatus} from '../lib/store.mjs';

const writeFacts=async(repo,rows)=>{
  await mkdir(resolve(repo,'dev-map'),{recursive:true});
...
test('a malformed row is named by its line and does not remove the rows around it',async t=>{
  ...
  assert.deepEqual((await readFacts({repo:fixture.repo})).errors,[{line:0,row:'',reason:'Missing dev-map/facts.tsv.'}]);
});
