...
const schemas = {
  ...
  'open-print': {library: string, 'no-open': boolean,'agent-owner':string},
  'create-preview': {library: string, recipe: string, stl: string, kind: string, machine: string, units: string, 'no-open': boolean,'agent-owner':string},
...
export const help = {
  commands: {
    ...
    'start-tour [--start-at-layer 12] [--no-open] [--agent-owner ID]': 'Fresh tour copies, live Studio, browser dispatch and participation context.',
    'open-print DIRECTORY [--no-open] [--agent-owner ID]': 'Open saved geometry/toolpath and return current recipe/review state.',
    'create-preview DIRECTORY [--recipe FILE | --stl FILE] [--machine ID] [--units auto|mm|inch] [--no-open] [--agent-owner ID]': 'Create/import unapproved geometry, open Studio and report assumptions.',
