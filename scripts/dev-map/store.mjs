...
async function inputHashes(repo) {
  const generator=await readdir(resolve(repoRoot,'scripts/dev-map'));
  const entries=await Promise.all(generator.filter(f=>f.endsWith('.mjs')).sort(order).map(async file=>
    [`generator:scripts/dev-map/${file}`,sha(await readFile(resolve(repoRoot,'scripts/dev-map',file)))]));
  for(const file of ['package-lock.json','maps/facts.tsv',...await compositionFiles(repo),'DECISIONS.md']) {
