...
export function compactPage(page, {code = false} = {}) {
  ...
  if (sourceOnly) {
    ...
    return clean(Object.fromEntries(keys.filter(key => key in page).map(key => [key, page[key]])), undefined, true);
  }
  ...
  // Incoming callers and their visible/off-page presentation share identities;
  // keep each relation once while retaining the rendering-specific references.
  if (page.calledFrom) {
    const represented = new Set([
      ...(page.callerReferences ?? []).map(ref => ref.index),
      ...(page.callerWires ?? []).map(wire => wire.from)
    ]);
