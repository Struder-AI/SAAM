...
export async function compositionFiles(repo) {
  const entries=await readdir(resolve(repo,'maps/flows'),{withFileTypes:true})
    .catch(error=>{if(error.code==='ENOENT')return [];throw error;});
  return ['maps/flows.json',...entries.filter(e=>e.isFile()&&e.name.endsWith('.json'))
    .map(e=>`maps/flows/${e.name}`).sort()];
}
export async function readCompositions({repo}={}) {
  ...
  for(const file of await compositionFiles(repo)) {
    const text=await readFile(resolve(repo,file),'utf8').catch(error=>{
      if(error.code==='ENOENT'&&file==='maps/flows.json')return null;throw error;
    });
  ...
  for(const spec of config.flows) {
    ...
    for(const g of groups) {
      ...
      for(const w of base) {
        ...
        const originalPort=[...(packet.inputs??[]),...(packet.outputs??[])].find(p=>p.port===outside);
      ...
      groupPages.set(g.index,{flow:true,generated:true,index:g.index,path:g.path,kind:'group',label:g.label,
        ...
        composition:{source:spec.source??'maps/flows.json',authored:['grouping','labels'],relations:'generated'},
