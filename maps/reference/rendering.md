...
Display geometry, material rendering, playback and performance. See [Studio](studio.md) for launch and review behavior.
...
The [machine presentation contract](presentation.md#implemented-consumer) owns
...
Studio uses the shared [bundle identity and reuse contract](lifecycle.md#print-bundle-and-current-formats).
...
**Failures.** Missing deposition frames, stationary events and unavailable machine models need explicit supported display behavior rather than invented beads/poses. Proxy topology is not authoritative manufacturing geometry. Detail settings cannot silently simplify emitted source or alter volume.

**Change together.** Coordinate move-store fields, playback cutoffs, model frames, camera fit/resize and material/toolpath settings. Keep worker/session identity and resource disposal intact through asynchronous builds.

**Verification.** Check camera axes/scale, bead dimensions, curved/vertical frames, mesh creases, source-time cutoffs and repeated rebuild/disposal. Compare displayed geometry to interpreted commands, not just screenshots. Checks: [studio-camera.test.mjs](../../core/tests/studio-camera.test.mjs), [studio-material.test.mjs](../../core/tests/studio-material.test.mjs), [studio-detail.test.mjs](../../core/tests/studio-detail.test.mjs), [studio-geometry.test.mjs](../../core/tests/studio-geometry.test.mjs), [studio-kinematics.test.mjs](../../core/tests/studio-kinematics.test.mjs).
