...
[machine output](output.md) owns source interpretation;
[print lifecycle](lifecycle.md) owns review and delivery.
...
[source-worker.mjs](../../studio/source-worker.mjs) decodes source once and retains compact
motion for the provider. [machine-session.mjs](../../studio/machine-session.mjs) owns bounded
...
[machine-view.mjs](../../studio/machine-view.mjs) compiles every v1 primitive, applies resolved
...
## Changing robot kinematic models

Sources: [dobot-kinematics.mjs](../../core/machine/dobot-kinematics.mjs), [denso-kinematics.mjs](../../core/machine/denso-kinematics.mjs).

**Contract.** Kinematic models translate the declared machine geometry and joint conventions into presentation poses. Solvers must retain branch/continuity policy and report reachability/constraint results in the provider contract. A nominal model supports review and jogging; it is distinct from controller execution and measured calibration.

**Failures.** Unreachable/singular configurations and exceeded limits must be explicit, not replaced by an apparently valid pose. Do not promote a visual solution into evidence of collision clearance or installed-machine accuracy.

**Change together.** Review machine dimensions/joint limits, rigid frame conventions, provider sampling and jog controls together. Preserve branch continuity across incremental and out-of-order requests.

**Verification.** Exercise forward/inverse consistency, branch boundaries, unreachable targets, singularities and frame transformations using known model cases. Checks: [dobot-kinematics.test.mjs](../../core/tests/dobot-kinematics.test.mjs), [studio-kinematics.test.mjs](../../core/tests/studio-kinematics.test.mjs), [machine-presentation.test.mjs](../../core/tests/machine-presentation.test.mjs).


## Changing presentation and jog contracts

Sources: [presentation.mjs](../../core/machine/presentation.mjs), [jog.mjs](../../core/machine/jog.mjs).

**Contract.** Presentation providers expose incremental source-time snapshots, model descriptors and lifecycle disposal through the map-owned provider contract. Jogging validates explicit requested changes against model/machine constraints and returns proposed state for review. Provider identity and availability are separate from source validity and playback time.

**Failures.** Stale asynchronous results cannot replace a newer binding. Unsupported presentation or invalid jog targets produce explicit unavailable/failure state without corrupting the interpreted source. Disposal must release retained work/resources.

**Change together.** Coordinate source workers, machine sessions, rendering, joint conventions and UI jog controls. Changing snapshot fields requires updates to both validation and every consumer.
...
inverse uses the transpose. The rigid axis-angle helper takes radians, while
source Euler/rotary fields explicitly named Deg use degrees. Local -Z points
