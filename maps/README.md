...
Other authored inputs are the scan scope, `scripts/dev-map/scope.mjs`, and
...
Off-page reference arrows start on the shared box and point outward. On an
expanded function page, they attach to its enclosing frame. Compatible value
...
Run `node scripts/dev-map.mjs watch-freshness` alongside a served or local viewer
...
base file and sorted `maps/flows/*.json` fragments; duplicate page specifications
fail. Adding, changing or removing a fragment invalidates freshness. The retained
`comparison-path-groups.json` is historical comparison, outside active inputs.
...
```sh
node --test core/tests/dev-map-flow.test.mjs core/tests/dev-map-flow-truth.test.mjs core/tests/dev-map-operators.test.mjs core/tests/dev-map-invocation.test.mjs core/tests/dev-map-callback-integrity.test.mjs core/tests/dev-map-store-integrity.test.mjs core/tests/dev-map-composition.test.mjs core/tests/dev-map-view.test.mjs
```
