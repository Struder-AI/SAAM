...
import {composePages,readCompositions} from '../../scripts/dev-map/composition.mjs';
import {generate,readGenerated,readCode,storeStatus} from '../../scripts/dev-map/store.mjs';
...
test('composition fragments invalidate stored reads on add, edit and removal and preserve nested source access',async t=>{
  ...
  await mkdir(resolve(repo,'core'));await mkdir(resolve(repo,'maps/flows'),{recursive:true});
  ...
  await generate({repo});
  const fragment='maps/flows/core.json',spec={schema:1,flows:[{path:'core',groups:[{id:'job',members:['core/work.mjs'],groups:[{id:'prepare',members:['core/work.mjs::a','core/work.mjs::b']}]}]}]};
