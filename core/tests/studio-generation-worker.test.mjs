...
test('preparation errors retain their notification and code; a changed plan takes precedence on generation',async()=>{
  ...
  const bundle={async loadBundle(){calls.push('load');return {generationHash:'locked'};},async checkPathBundle(){calls.push('check');throw failure;},async generateBundle(){assert.fail('failed preparation must not write');}};
  ...
  bundle.loadBundle=async()=>({generationHash:'changed'});
...
test('worker cancellation and checked-source failures retain exact error responses',async()=>{
  ...
  const cancelled={async loadBundle(){return {generationHash:'locked'};},async generateBundle(){assert.fail('cancelled calculations must not write');}};
  ...
  for(const [source,error] of [[{},'Checked machine source is unavailable.'],[{program:{},programError:'SYNTHETIC byte mismatch'},'SYNTHETIC byte mismatch']]){
    const bundle={async loadBundle(directory,read){return read.program===false?{generationHash:'locked'}:source;},async generateBundle(directory,write){assert.equal(write.development,false);return {};}};
...
test('preparation succeeds without persisting output and progress keeps stage/percent throttling and cancellation',async()=>{
  ...
  const bundle={async loadBundle(directory,read){calls.push(['load',directory,read]);return {generationHash:'locked'};},async checkPathBundle(directory,{onProgress}){calls.push(['check',directory]);onProgress({stage:'Checking',completed:1,total:100});},async generateBundle(){assert.fail('preparation must not persist');}};
