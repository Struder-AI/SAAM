...
import {generate,readGenerated,readCode,readIndex,storeDir,storeStatus} from '../../scripts/dev-map/store.mjs';
import {viewModel} from '../../scripts/dev-map/generated-view.mjs';
...
export function main(x){return leaf(x)+first(x);}`});
  await f.put('maps/flows.json',JSON.stringify({schema:1,flows:[{path:'core/callers.mjs::main',groups:[
...
test('a declaration with disconnected local helpers opens source rather than an empty explanatory graph',async t=>{
...
first();second();
  ...
  assert.equal(page.wires.length,0);
  assert.equal(page.destination,'code');
  assert.match(page.source,/export function schedule/);
});
...
test('facts, authored grouping and generator/dependency fingerprints invalidate reads',async t=>{
  const f=await fixture(t,pair);await f.run();
  await f.put('maps/flows.json','{"schema":1,"flows":[]}');
  await f.put('maps/facts.tsv','target\tkind\tvalue\tsource\n');
  ...
  assert.deepEqual((await f.page('0')).stale.inputs.sort(),[generator,'maps/flows.json','maps/facts.tsv','package-lock.json'].sort());
});
...
test('context groups retain source identities through storage, scoped recomposition, and code reads',async t=>{
  ...
  await f.put('maps/flows.json',JSON.stringify({schema:1,flows:[{path:'core/alpha',groups:[
...
test('changed authored structural scopes widen regeneration instead of mixing new nodes with old addresses',async t=>{
  ...
  await f.put('maps/flows.json',JSON.stringify({schema:1,flows:[{path:'core/beta',groups:[
