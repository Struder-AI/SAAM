...
import {generate,readCode,storeDir} from '../../scripts/dev-map/store.mjs';
...
test('value bundling preserves separate conditions, operator roles and call semantics',()=>{
  ...
  const probe=spawnSync('python',['-c',
    'import importlib.util,json,sys; sys.path.insert(0,"scripts/dev-map"); s=importlib.util.spec_from_file_location("generated_view","scripts/dev-map/generated-view.py"); m=importlib.util.module_from_spec(s); s.loader.exec_module(m); wires=json.loads(sys.argv[1]); before=json.dumps(wires); out=m.value_bundles(wires); print(json.dumps({"out":out,"unchanged":before==json.dumps(wires)}))',
...
test('off-page reference arrows point outward from boxes and from the expanded owner frame',async t=>{
...
export const outside=x=>main(x);`});
  ...
  const own=svg.match(/class="fm-caller-arrow fm-owner-reference"[^>]*d="M([\d.]+),([\d.]+) L([\d.]+),([\d.]+)"/);
  assert.ok(frame&&own);assert.equal(+own[2],+frame[1]+ +frame[2]);assert.ok(+own[4]>+own[2]);
...
export function omitted(){return stage();}`});
  ...
  for(const ref of input.references.filter(r=>r.index===caller.index))
    assert.ok(svg.includes(`>${caller.index}:${ref.line}:${ref.column}</text>`),'call site is addressable separately');
