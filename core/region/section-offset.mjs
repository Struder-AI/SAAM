...
export function offsetSurfaceSection(patch,chain,deltaMm,{mode='horizontal',side=1,offsetTightness=1,toleranceMm=0.01,maxStepMm=0.5}={}) {
  requireThat(['horizontal','normal'].includes(mode)&&[1,-1].includes(side),'Invalid section-offset mode or side.');
  requireThat(Number.isFinite(deltaMm)&&deltaMm>=0&&toleranceMm>0&&maxStepMm>0,'Invalid section-offset distance or sampling settings.');
  ...
  const split=(a,b)=>{
    ...
    requireThat(u>Math.min(a.u,b.u)&&u<Math.max(a.u,b.u),'Rimming offset cannot resolve its requested tolerance: the section interval is one representable step in u, so the reference surface steps there.');
    return [...split(a,mid),...split(mid,b)];
  };
