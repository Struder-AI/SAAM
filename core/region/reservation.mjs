...
export function surfaceStroke({points2d,z,nominalHeightMm,widthMm,surface,closed=false,
  ...
  requireThat(Number.isFinite(maxStepMm)&&maxStepMm>0&&Number.isFinite(toleranceMm)&&toleranceMm>0&&Number.isInteger(maxSegments)&&maxSegments>0&&maxSegments<=100000,'Invalid surface sampling limits.');
  ...
  let samples=0;
  const sample=point=>{
    requireThat(++samples<=maxSegments*12,'Lower-surface sampling budget exceeded; simplify the interface or increase its locked sampling budget.');
  ...
  const append=(a,b,length,error)=>{
    requireThat(volumesMm3.length<maxSegments,'Lower-surface segment budget exceeded.');
  ...
  const split=(a,b,depth)=>{
    ...
    requireThat(depth<=30,'Lower-surface gap does not converge within the sampling depth limit.');
    ...
    if(length>maxStepMm||error>toleranceMm){split(a,probes[1],depth+1);split(probes[1],b,depth+1);}
    else append(a,b,length,error);
  };
  let previous=sample(source[0]);
  for(let i=1;i<source.length;i++){const next=sample(source[i]);split(previous,next,0);previous=next;}
