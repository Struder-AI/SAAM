...
const MAX=64_000_000, MAX_ENTRIES=64;
...
export function packZip(entries){
  ...
  for(const [name,value] of [...entries].sort(([a],[b])=>a<b?-1:a>b?1:0)){
    ...
    total+=bytes.length;requireThat(total<=MAX,'ZIP contents exceed size limit.');
  ...
  const bytes=Buffer.concat([...local,cd,end]);requireThat(bytes.length<=MAX,'ZIP archive exceeds size limit.');return bytes;
}
export function unpackZip(input){
  const b=Buffer.from(input);requireThat(b.length>=22&&b.length<=MAX,'Invalid ZIP size.');
  ...
  const result=new Map(),ranges=[];let p=start,total=0;
  for(let i=0;i<count;i++){
    ...
    total+=length;requireThat(total<=MAX,'ZIP contents exceed size limit.');
