...
const ENVELOPE_HASHES={
  "h2d-saam-startup-v4": "c037a5482cd034a13c1836eca608f83e0c79e7e92ddf4704dcb159e591fd1ab3",
  "x1c-saam-startup-v2": "17bca8d958ca53fc2d694f57821f6efd1a130cce5276f4401477c8935fbda215"
};
...
function header(c,program,job){
  return ['; HEADER_BLOCK_START',`; generated by SAAM ${c.release.generatorVersion}`,`; build date: ${c.release.buildDate}`,
    ...
    `; filament_diameter: ${job.filamentMm}`,'; filament: 1','; HEADER_BLOCK_END','',
...
export function exportAndInterpretBambu(path,plan,machine,release){
  const output=configuration(plan,machine),job=resolveBambuJob(plan,machine,output);
...
export function interpretBambu(bytes,plan,machine){
  const output=configuration(plan,machine),job=resolveBambuJob(plan,machine,output),entries=unpackZip(bytes);
  ...
  const body=code.slice(begin+BEGIN.length,end),program=interpretBody(body,plan,machine),s=sections(c,job,output);
...
function completeProgram(program,code,c,s,job){
  ...
  const mapping=tray?`Requested AMS ${tray.unit}, slot ${tray.slot}: confirm the printer maps this job's filament to that tray before starting.`:
    'No physical AMS tray was specified; confirm the spool on the printer before starting.';
  program.envelope.job={tool:job.tool,nozzleMm:job.nozzle,nozzleDiametersMm:job.nozzles.map(Number),plate:job.plate.name,
    logicalFilament:job.used,filamentColor:job.color,requestedTray:tray,amsConnections:job.amsConnections};
  program.summary.startup=program.envelope.notice+' '+mapping;
...
function packageEntries(code,c,program,plan,output,job){
  ...
  const plate={bbox_all:bbox,bbox_objects:[{area:(bbox[2]-bbox[0])*(bbox[3]-bbox[1]),bbox,id:1,layer_height:plan.process.layerMm,name:'SAAM part'}],
    bed_type:job.plate.id,filament_colors:[color],filament_ids:[used],first_extruder:used,first_layer_time:0,is_seq_print:false,nozzle_diameter:nozzle,version:2};
  const entries=new Map([
    [GCODE,code],[GCODE+'.md5',digest(code,'md5')],['Metadata/saam.json',json(c)],['Metadata/saam-job.json',json({schema:'saam-bambu-job/1',tool,nozzles,plate:job.plate.id,logicalFilament:used,filaments:job.filaments,requestedTray:job.requestedTray,amsConnections:job.amsConnections,amsMapping:'Confirm logical filament to physical tray mapping on the printer before starting.'})],
