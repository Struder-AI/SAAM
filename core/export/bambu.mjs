...
const ENVELOPE_HASH='2e4b819716e4ee42d1e10c25b0bc64db29a12389c269507befd4db2db1cead32';
...
function interpretBody(body,plan,machine){
  ...
  const program=interpretMotion(body,plan,machine),bounds=toolBounds(machine,plan.setup.tool);
...
export function exportBambu(path,plan,machine,release){
  ...
  const body=prelude(plan)+exportMotion(path,plan).map(l=>l==='M107'?'M106 S0':l).join('\n')+'\n';
