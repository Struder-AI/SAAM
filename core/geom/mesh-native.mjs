...
async function readOff(path,{signal,progress}){
  ...
  try{for await(const line of lines){signal?.throwIfAborted();const words=line.trim().split(/\s+/);if(!line.trim()||words[0].startsWith('#'))continue;
    ...
    if(header===1){nv=Number(words[0]);nf=Number(words[1]);checkMeshBudget(nv,nf);header++;continue;}
...
export async function repairMeshNative(mesh,{maxHoleEdges=0,maxHoleDiameterMm=0,timeoutMs=120000,signal,progress=()=>{}}={}){
  ...
  if(!Number.isSafeInteger(timeoutMs)||timeoutMs<1)throw Error('Repair timeoutMs must be a positive integer.');
  ...
  try{
    ...
    const report=await new Promise((yes,no)=>{
      ...
      let stdout='',stderr='',lines='',failed;const timeout=setTimeout(()=>{failed=Object.assign(Error(`Mesh repair exceeded ${timeoutMs} ms; no result accepted.`),{code:'MESH_REPAIR_TIMEOUT'});child.kill();},timeoutMs);
      child.stdout.on('data',data=>{stdout+=data;if(stdout.length>65536){failed=Error('Native repair report exceeded its limit');child.kill();}});
