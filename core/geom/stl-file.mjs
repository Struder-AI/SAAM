...
async function decodeFile(path,{units,scale=1,signal,progress=()=>{}}={}){
  ...
  if(binary)checkMeshBudget(count*3,count);
  const factor=scale*(units==='inch'?25.4:1),vertices=[],triangles=[],lookup=new Map(),sha=createHash('sha256');let bytesRead=0;
  function facet(points){const t=points.map(p=>{p=p.map(v=>v*factor);if(!p.every(Number.isFinite))throw Error('Nonfinite STL coordinate.');const key=p.join(',');if(!lookup.has(key)){lookup.set(key,vertices.length);vertices.push(p);}return lookup.get(key);});triangles.push(t);if(triangles.length%4096===0)checkMeshBudget(vertices.length,triangles.length);}
  ...
  for await(const data of createReadStream(path,{highWaterMark:65536,signal})){
  ...
  checkMeshBudget(vertices.length,triangles.length);return {vertices,triangles,sha256:sha.digest('hex'),bytes:size};
}
