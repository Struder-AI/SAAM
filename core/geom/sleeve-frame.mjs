...
export function prepareLooseSleeveOffsets({patch,rangeMm}){
  ...
  const frameAt=(phase,z,tightness=0)=>{
    const frame=offsets.frameAt(...parameters(phase,z),tightness);
  ...
  return {frameAt,offsetPatch:offsets.offsetPatch,at:(u,z,depth=0,tightness=0)=>{
    ...
    const point=offsets.at(...parameters(u,z),depth,tightness);
    requireThat(Math.abs(point[2]-z)<=1e-8,'Horizontal sleeve offset requires patch V to reproduce actual Z.');
