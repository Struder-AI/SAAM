...
export function substrateLoops(plan){
  ...
  const width=plan.process.lineWidthMm,count=Math.max(2,Math.round((outer-inner)/width));
...
export function pipeCladdingResult({plan,shell,after=[],id='pipe-cladding'}){
  ...
  for(let shell=0;shell<s.shells;shell++){
    ...
    if(axial){
      ...
      const tracks=2*Math.ceil(circumference/p.lineWidthMm/2),width=circumference/tracks;
      ...
      const pitch=p.lineWidthMm,start=angle,turns=(g.heightMm+pitch)/pitch;
      const count=Math.ceil(turns*360/angularStep),stroke=makeStroke(radius,'circumferential');
      stroke.volumesMm3=[];
      for(let k=0;k<=count;k++){
        const advance=k/count*turns*pitch,z=-pitch/2+advance;
        ...
        append(stroke,radius,start+advance/pitch*360,Math.max(pitch/2,Math.min(g.heightMm-pitch/2,z)));
        if(k){
          const mid=-pitch/2+(k-.5)/count*turns*pitch;
          const width=Math.max(0,Math.min(pitch,mid+pitch/2,g.heightMm+pitch/2-mid));
          const a=stroke.points[k-1],b=stroke.points[k];
          stroke.volumesMm3.push(Math.hypot(...b.map((v,i)=>v-a[i]))*width*s.normalMm);
        }
      }
      angle=start+turns*360;strokes.push(stroke);
    }
    const operationId=id+':'+shell;
    operations.push({id:operationId,layerId:operationId,phase:axial?'cladding-axial':'cladding-hoop',layer:shell,rank:radius,
