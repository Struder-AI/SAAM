...
export function surfaceCladdingResult({shell,plan,after=[],id='pipe-cladding'}){
  const s=plan.skills['pipe-cladding'],p=plan.process,chart=surfaceRegion(shell,s.surface),w=p.lineWidthMm;
  ...
  let points=0,angle=0,previous=after;const operations=[];
  ...
  for(let layer=0;layer<s.shells;layer++){
    const offset=(layer+.5)*s.normalMm,strokes=[],axial=layer%2===0;
    if(axial){
      for(let sector=0;sector<cuts.length-1;sector++){
        ...
        const lengths=vs.map(v=>ring(v).length),count=Math.ceil(Math.max(...lengths)/w);
        requireThat(count*vs.length<s.maxPoints,'Surface course survey exceeds maxPoints.');
        for(let k=0;k<count;k++){
          const threshold=k*w;
          const centerAt=v=>{
            const r=ring(v),width=Math.max(0,Math.min(w,r.length-threshold)),arc=Math.min(r.length,threshold+width/2);
      ...
      const turns=Math.ceil(meridianMax/w)+1,pitch=1/(turns-1);
      ...
      const perTurn=Math.max(64,Math.ceil(circumferenceMax/s.sampleStepMm)),count=turns*perTurn,stroke=newStroke('circumferential');
      ...
      const uvAt=progress=>{const raw=-pitch/2+progress*pitch;return [progress-Math.floor(progress),Math.max(v0,Math.min(v1,raw))];};
      for(let i=0;i<count;i++){
        ...
        const section=sampleSurfaceCurve(chart,t=>uvAt((i+t)/perTurn),offset,options);
        for(const e of section.slice(i?1:0)){
          const rawV=-pitch/2+(i+e.t)/perTurn*pitch,local=Math.hypot(...e.dv);
          samples.push(e);widths.push(Math.max(0,Math.min(pitch,rawV+pitch/2,1+pitch/2-rawV))*local);
    ...
    operations.push({id:operationId,layerId:operationId,phase:axial?'cladding-axial':'cladding-hoop',layer,rank:layer,after:previous,
