import { VERSION, distance, requireThat, number } from './model.mjs';
...
export function exportGcode(path,plan,machine) {
  ...
  const lines=[';START_OF_HEADER',';HEADER_VERSION:0.1',';FLAVOR:Griffin',';GENERATOR.NAME:SAAM',`;GENERATOR.VERSION:${VERSION}`,
    ...
    ';BUILD_PLATE.TYPE:glass',`;BUILD_PLATE.INITIAL_TEMPERATURE:${s.bedC}`,`;PRINT.TIME:${Math.ceil(time)}`,';PRINT.GROUPS:1'];
  if(s.materialGuid)lines.push(`;EXTRUDER_TRAIN.${tool}.MATERIAL.GUID:${s.materialGuid}`);
  for(const [i,axis]of ['X','Y','Z'].entries())lines.push(`;PRINT.SIZE.MIN.${axis}:${fmt(min[i])}`,`;PRINT.SIZE.MAX.${axis}:${fmt(max[i])}`);
  lines.push(';END_OF_HEADER',';Clearance: operator responsibility for this demo',
    ';Firmware owns Griffin preflight and priming; see the locked setup.',
    `T${tool}`,'G21','G90','M82',`M190 S${s.bedC}`,`M109 T${tool} S${s.nozzleC}`,
    'G280 S1','G0 Z20 F300','G92 E0');
  let e=0,tag='';
  for(const a of path.actions) {
...
export function interpretGcode(text,plan,machine) {
  ...
  let pos=[...machine.tools[s.tool].startupXY,machine.startup.zAfterPrimeMm],e=0,feed=0,absolute=null,absE=null,metric=false;
  let tool=null,nozzle=0,bed=0,hot=false,bedReady=false,fan=0,debt=0,prime=false,phase='startup',layer=-1,time=0,volume=0;
  ...
  for(const [index,raw]of text.split(/\r?\n/).entries()) {
    ...
    switch(command) {
      ...
      case 'T0':case 'T1':only('');requireThat(Number(command[1])===s.tool && !prime,'Unexpected tool change.');tool=Number(command[1]);break;
      case 'M190':case 'M140':
      ...
      case 'G280':
        only('S','S');requireThat(args.S===1&&tool===s.tool&&hot&&bedReady&&!prime&&!moves.length,'Unsupported Griffin priming state.');
        prime=true;events.push({line,kind:'firmware-prime',description:'G280 S1: firmware-controlled priming, no prime blob. Internal motion is not simulated.'});break;
      ...
      case 'G0':case 'G1': {
        ...
        requireThat(metric&&absolute!==null&&absE!==null&&prime,'Unknown initial motion state.');
  ...
  return {moves,events,header,summary:{moves:moves.length,extrusionMoves:moves.filter(m=>m.extruding).length,
    ...
    startup:'Firmware preflight/priming internals and heating time are not simulated.',clearance:'Operator responsibility; not checked.'}};
}
