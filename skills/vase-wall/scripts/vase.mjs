...
import {offsetRegion,loopArea,pointInRegion,dedupe,pointSegmentDistance} from '../../../core/region/region2d.mjs';
import {requireThat,distance} from '../../../core/geom/tolerance.mjs';

export const VASE_WALL_DEFAULTS={zStartMm:0,zEndMm:null,endTransition:'spiral',sampleStepMm:1,toleranceMm:0.02,minFeatureMm:0.4,maxPoints:100000};
...
export function vaseWallResult({shell,plan,machine,id='vase-wall',after=[],zStartMm=null,zEndMm=null}) {
  ...
  requireThat(settings.toleranceMm<width/4,'Vase tolerance must be smaller than one quarter of the bead width.');
  const cache=new Map();let center,nudgedSections=0;
  function section(z) {
    const key=z.toFixed(10);if(cache.has(key))return cache.get(key);
    requireThat(cache.size<settings.maxPoints*4,'Vase section query budget exceeded; shorten the wall or increase the locked maxPoints.');
    const cut=sectionGeometry(shell,z,{minFeatureMm:settings.minFeatureMm});
    requireThat(Math.abs(cut.nudgedByMm??0)<=settings.toleranceMm,'Vase section nudge exceeds the locked tolerance.');
    if(cut.nudgedByMm)nudgedSections++;
    const outer=convexLoop(cut.loops),loop=convexLoop(offsetRegion([outer],-width/2,{arcToleranceMm:settings.toleranceMm/4}));
  ...
  function point(t) {
    ...
    requireThat(Math.abs(standoff-width/2)<=settings.toleranceMm,'Vase centerline does not preserve the declared bead-width boundary within tolerance.');
    if(t>1) {
      const belowZ=zAt(t-1),below=rayPoint(section(belowZ).loop,center,t*2*Math.PI);
      requireThat(distance(xy,below)<=width-settings.toleranceMm,'Vase sections shift too far per turn for bead overlap; a gentler taper is required.');
    }
    return [...xy,z];
  }
  ...
  function append(a,b,pa,pb,depth=0) {
    ...
    requireThat(points.length<settings.maxPoints,'Vase point budget exceeded; shorten the wall or increase the locked maxPoints.');
  ...
  return {id,operations:[{id:id+':wall',layerId:id+':continuous',phase:'vase-wall',layer:0,rank:start,
    ...
    report:{startMm:start,endMm:end,baseTopMm:base,turns,spiralTurns,endTransition:settings.endTransition,levelRimMm:settings.endTransition==='level'?end:null,points:points.length,sectionQueries:cache.size,nudgedSections,
      volumeMm3:volumesMm3.reduce((sum,v)=>sum+v,0),speedMmS:speed,maximumAngleDeg,
      scope:'One convex outer section with a persistent interior origin; sampled topology and bead overlap checks, no physical validation.'}};
}
