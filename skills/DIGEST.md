...
| [line-network](line-network/SKILL.md) | Print sparse planar frames and trusses directly from explicit centerline polylines, including layer-specific reinforcement strokes, without filling their enclosing area. |
| [bridging](bridging/SKILL.md) | Generate experimental straight XYZ bridge spans between two sampled supporting rims, with alternating continuous or one-way diagnostic paths and independently controlled attachment motions. Compose with existing wall producers; this skill generates no walls and has no physical validation. |
...
| [bed-adhesion](bed-adhesion/SKILL.md) | Hold a part down when its own first layer is too small or too thin to grip the bed, as on an open-bottom vase-mode print. One entry so far - a single-layer brim of concentric loops printed around the part outline, wider and slower than the part itself. |
