---
name: bridging
description: Generate experimental straight XYZ bridge spans between two sampled supporting rims, with alternating continuous or one-way diagnostic paths and independently controlled attachment motions. Compose with existing wall producers; this skill generates no walls and has no physical validation.
---

# Bridging

Use the shared print bundle workflow. Enable `skills.bridging` globally alongside
standard wall producers, including regional planar-infill. Bridge operations
follow all model operations, then run in the listed order. No walls, floors or
roof infill are generated by this skill. Bridges do not publish a supporting
surface for later pattern consumers.

For one or two walls around a solid guide, use planar-infill `perimeters: 1` or
`2`, `density: 0`, and disable full-fill, draped-skin and vase-wall. Full-fill
`solid-surfaces` with zero top/bottom layers is also compatible. Body full-fill
cannot leave an empty interior. Geometry guides and printed material differ;
review actual toolpaths, including complete wall circuits.

## Recipe

`maxExcursionMm` bounds each whole bridge including endpoint presses (default
10 mm). It is a user-selected geometric constraint, not a head collision model.
No 15-degree slope limit is applied. Machine capability and XYZ limits still apply.

Each entry in `bridges` has these required fields:

| Field | Meaning |
|---|---|
| `id` | Unique lowercase identifier |
| `rails` | Two equally sized arrays of XYZ gap-edge points, at bridge deposition height; XY is relative to plan placement |
| `mode` | `alternating` reverses successive spans and extrudes between anchors; `one-way` returns through shared retracted travel |
| `overlapMm` | XY extension from each gap edge into its supporting wall |
| `speedMmS` | Unsupported span speed |
| `attachmentSpeedMmS` | Supported attachment, turn, lead, jog and press speed |
| `flowMultiplier` | Unsupported volume multiplier only; reference area is line width times layer height, integrated along 3D length |
| `pressMm` | Down-and-up extruding dip at both supported anchors; zero disables |
| `jogMm` | Along-rim out-and-back excursion before each span; zero disables |
| `leadInMm` | Supported along-rim lead before every span; zero disables |

The author supplies actual gap edges and rim heights. Every free span is one
straight segment; attachment paths remain separate. Turns follow chords between
successive anchor samples. Jog/lead direction follows the local sampled rail;
sample curved rails finely and keep these motions within supporting material.
The first rail is the one-way start rail and first alternating start rail.

Generation checks the complete XYZ excursion, bounds, exact continuity, and
sampled attachment coverage against emitted planar wall/fill strokes one layer
below. Coverage allows 0.015 mm numerical tolerance; it is not adhesion evidence.
Unsupported spans are not automatically checked for intervening objects or
swept-head clearance. No temperature sweep or endpoint dwell control is exposed
yet; temperature and fan use the common process settings.

Physical sag, tension, attachment and pressure behavior remain untested. Save
the exact recipe, generated-program hash, photos and operator report per run.
Review geometry, process and generated paths in Studio before ordinary delivery.
