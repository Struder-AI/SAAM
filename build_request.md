...
- Remaining: In the order given in [dev-map/HANDOFF.md](dev-map/HANDOFF.md#what-remains-in-order): class instance state; thoughtful root clustering. Done 2026-09-21/22: the invocation edge, callback targets as references, array-method callbacks entered, closure-owned state, leaves drawn on the map above, finding rows on function pages, loop accumulation, the repeated-box ruling, the two code-shape sites, the island review.
