...
- Remaining: In the order given in [dev-map/HANDOFF.md](dev-map/HANDOFF.md#what-remains-in-order): leaves drawn on the map above; loop accumulation; class instance state; finding rows on function pages; thoughtful root clustering. Done 2026-09-21/22: the invocation edge, callback targets as references, array-method callbacks entered, closure-owned state, the repeated-box ruling, the two code-shape sites, the island review.
