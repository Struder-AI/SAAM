...
- Remaining: In the order given in [dev-map/HANDOFF.md](dev-map/HANDOFF.md#what-remains-in-order): thoughtful root clustering (phase 1 awaiting review). Done 2026-09-21/22: the invocation edge, callback targets as references, array-method callbacks entered, closure-owned state, leaves drawn on the map above, finding rows on function pages, loop accumulation, class instance state, the repeated-box ruling, the two code-shape sites, the island review.
