...
- Latest steering (2026-09-21): user prioritizes the next experiment on H2D same-nozzle AMS colours, blue → orange → blue through right 0.8, fast startup and no tower. v11 was compared with the supplied same-nozzle Studio export; main and requested 0.8/0.8 ALT were copied to D: with hash verification. Both request blue/orange/blue through right 0.8 but the user reports both print entirely orange; the Studio same-nozzle reference physically prints both colours. AMS-09 physically changed colours after replacing only the failed ALT archive’s project entry, with all other entries held fixed. It declared 0.8/0.8. AMS-10 changes only left-diameter declarations to 0.4; the user confirms it works. Mixed installed diameters are therefore not the cause of this same-nozzle colour failure. AMS-11-L04-R08 is generated from the shared v12 exporter with byte-identical executable commands to AMS-10, copied to D: and hash-verified; it failed entirely orange. The v12 authored project writer passes software checks but fails this physical colour test. AMS-13/14/15 now independently reduce working AMS-10: stored templates, saved routing and variant tables. All are on D: with verified hashes. AMS-13 failed entirely orange, proving that emptying the stored template family alone breaks working AMS-10. Independent routing and Standard-only variant results AMS-14/15 passed. AMS-16/17 isolate startup/change templates; AMS-18 substitutes SAAM's own already-tested startup text into the startup field. These diagnostic candidates are prepared locally. Shared v13 now reuses the exact authored executable startup/shutdown strings in project fields. Normal-exporter AMS-19/DUAL-20 pass strict reopen and preserve successful executable hashes; their physical acceptance is pending. The required project fields remain unidentified individually. Dual-nozzle work remains open.
