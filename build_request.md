...
- Latest steering (2026-09-21): user prioritizes the next experiment on H2D same-nozzle AMS colours, blue → orange → blue through right 0.8, fast startup and no tower. v11 was compared with the supplied same-nozzle Studio export; main and requested 0.8/0.8 ALT were copied to D: with hash verification. Both request blue/orange/blue through right 0.8 but the user reports both print entirely orange; the Studio same-nozzle reference physically prints both colours. AMS-09 physically changed colours after replacing only the failed ALT archive’s project entry, with all other entries held fixed. It declared 0.8/0.8. AMS-10 changes only left-diameter declarations to 0.4 and is on D: with its hash verified; result pending. The v12 authored project writer and maker guidance are implemented with software checks, but a freshly generated colour job still needs physical acceptance. The required project fields remain unidentified individually. Dual-nozzle work remains open.
